...
Phase 0 through Phase 8 are implemented (`src/seoulkit_studio/schema/`,
...
`render/overlay.py`, and `render/audio_mix.py`; 273/273 tests passing as
of this update, with the ffmpeg/ffprobe-requiring subset auto-skipping
when those tools are absent). Phase 9 onward are not started and should
not be assumed to work - do not reimplement Phase
0/1/2/2.5/3/4/5/6/7/8 in a new session; extend from here.
...
- `render/hold.py::hold_clip()` cannot prevent being *called twice* on the
  ...
  enforce on its own. Real double-application prevention - deciding
  *whether* to call `hold_clip()` at all for a given segment - is the
  responsibility of the same not-yet-built assembly layer named in the
  `concat_clips()` ordering-contract gap below (likely Phase 9-11).
  `tests/test_hold.py::test_calling_hold_clip_twice_visibly_doubles_the_extension`
  doesn't prove prevention; it proves *detection*.
...
- `render/concat.py::concat_clips()` does not sort `clip_paths` - it joins
  ...
  deliberately has no knowledge of. This means **the assembly layer that
  turns `edit_plan.json`'s `segments[]` into an ordered `list[Path]` of
  Phase-3-trimmed files does not exist yet**, and until it does, nothing
  actually enforces the ordering contract end-to-end. The same not-yet-
  built assembly layer is also responsible for calling `overlay.py`/
  `subtitle.py`/`audio_mix.py` in the right pipeline position (ch. 09:
  trim → hold → concat → overlay → subtitle → audio mix → final encode) -
  nothing today enforces that ordering either.

  Worth remembering when that assembly layer gets built: a
  `sorted(clip_paths)` reorder is a *silent* bug - FFmpeg raised no error
  and produced a fully valid, fully wrong-order video. That's different
  from bugs FFmpeg itself refuses outright ("cannot edit existing files
  in-place", "unconnected output"). Ordering mistakes get no such safety
  net from FFmpeg - only a real-content test catches them.
...
- `compute_effective_status()` (Phase 2) returns only the `PlanStatus`
  string (`READY`/`REVIEW_REQUIRED`/`NOT_READY`) - there is no function
  that takes that *final*, blended status and returns Preview/Final
  `bool`s the way Phase 1's `severity_to_execution()` does for a raw
  severity. Not urgent before now, but Phase 9 (Preview/Final split, hard
  gate) will need exactly this - add a small
  `effective_status -> ExecutionPermission` mapping there, or reuse
