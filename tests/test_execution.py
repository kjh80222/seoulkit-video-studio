...
# --- plan_status_as_declared exposure (added for Phase 10 ch. 17) ----------


def test_plan_status_as_declared_is_exposed_even_when_it_diverges_from_effective(tmp_path):
    # Stage 4 declared READY, but a blocking issue (missing clip) forces
    # effective_status down to NOT_READY - ch.17's Render Report wants both
    # values recorded precisely so this divergence is visible after the fact.
    plan = base_plan()
    plan_path = tmp_path / "edit_plan.json"
    plan_path.write_text(json.dumps(plan))
    project_dir = tmp_path / "project"  # deliberately empty -> MISSING_CLIP

    result = evaluate_plan(plan_path, project_dir)

    assert result.plan_status_as_declared == "READY"
    assert result.effective_status == "NOT_READY"


def test_plan_status_as_declared_is_none_on_load_error(tmp_path):
    plan_path = tmp_path / "edit_plan.json"
    plan_path.write_text("{ this is not valid json")
    project_dir = tmp_path / "project"
    ...
    assert result.plan_status_as_declared is None
...
# --- SFX contract resolution wired through evaluate_plan() -----------------


def test_unresolved_sfx_candidate_downgrades_ready_to_review_required(tmp_path):
    plan = base_plan()
    plan["audio_layers"]["sfx_source"] = {"mode": "stage3_optional", "clips": []}  # Stage 4 never resolved it
    plan_path = tmp_path / "edit_plan.json"
    plan_path.write_text(json.dumps(plan))
    project_dir = make_project_dir(tmp_path, plan)
                ...
                "clips": [
                    {
                        ...
                        "key_event_end_ms": segment["key_event_end_ms"],
                        "settle_start_ms": segment["settle_start_ms"],
                        "optional_source_audio": {"available": True, "file": "clips/sfx/1a.wav"},
                    }
                ],
            }
        )
    ...
    assert any(issue.code == "SFX_UNRESOLVED_CANDIDATE" for issue in result.clip_manifest_issues)


def test_legacy_manifest_without_contract_version_raises_no_sfx_issues(tmp_path):
    # Same "candidate never resolved" shape as the test above, but the
    # manifest doesn't declare sfx_contract_version - it must not be judged
    # against a contract it never opted into.
    plan = base_plan()
    plan["audio_layers"]["sfx_source"] = {"mode": "stage3_optional", "clips": []}
    plan_path = tmp_path / "edit_plan.json"
    plan_path.write_text(json.dumps(plan))
    project_dir = make_project_dir(tmp_path, plan)
    ...
    assert result.effective_status == "READY"
    assert not any(issue.code in ("SFX_UNRESOLVED_CANDIDATE", "SFX_CONTRACT_FIELD_MISSING") for issue in result.clip_manifest_issues)
